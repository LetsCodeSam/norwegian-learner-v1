...
export function LessonTabs({ blocks }: { blocks: Block[] }) {
  ...
  // Keep track of the most recent mono title while iterating
  let lastMonoTitle: string | undefined;

  return (
    <div className="space-y-3">
      {/* Tabs row - sticky under the lesson header controls */}
      <div
        className="flex flex-wrap gap-2 sticky top-[88px] z-20 bg-gray-100/60 dark:bg-neutral-900/60 backdrop-blur px-2 py-2 rounded-lg"
        role="tablist"
      ...
      {/* Render only blocks matching the active tab.
          Also advance lastMonoTitle as we scan, so verbs can inherit it. */}
      {blocks?.map((b, i) => {
        if (!b?.kind) return null;

        // Record the latest monolog title seen so far
        if (b.kind === 'mono' && b.title) {
          lastMonoTitle = b.title;
        }
        ...
        switch (b.kind) {
         ...
         case 'verbs_table': {
          const scope: string | undefined = b.scope ?? lastMonoTitle;
          return tab === 'verbs_table'
            ? <VerbsTableBlock key={i} block={b as any} scope={scope} />
            : null;
        }
